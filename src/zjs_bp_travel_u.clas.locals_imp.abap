...
.
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_travel_failed TYPE TABLE FOR FAILED zjs_i_travel_u.
    TYPES tt_travel_reported TYPE TABLE FOR REPORTED zjs_i_travel_u.
    TYPES tt_booking_failed TYPE TABLE FOR FAILED zjs_i_booking_u.
    TYPES tt_booking_reported TYPE TABLE FOR REPORTED zjs_i_booking_u.
...
********************************************************************************
  METHOD get_instance_features.
    READ ENTITIES OF zjs_i_travel_u IN LOCAL MODE
    ENTITY Travel
...
**********************************************************************
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<travel_create>).
      ...
      map_messages( EXPORTING cid          = <travel_create>-%cid
                              messages     = messages
                    IMPORTING failed_added = DATA(failed_added)
...
**********************************************************************
  METHOD read.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<travel_to_read>) GROUP BY <travel_to_read>-%tky.
      ...
      map_messages( EXPORTING travel_id = <travel_to_read>-TravelID
                               messages = messages
                    IMPORTING failed_added = DATA(failed_added)
                    CHANGING failed = failed-travel
                             reported = reported-travel ).
